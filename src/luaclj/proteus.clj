...
(defn- add-meta [obj m]
  (with-meta obj (merge (or (meta obj) {}) m)))
...
(defn- transform-handler [x]
  ;(println "handler:" x "meta:" (meta x))
  (cond (:processed (meta x))
        x
...
(defmacro let-mutable
  ...
  (let [ks (->> bindings
        ...
        types (map
                #(condp = (typeof % &env)
                   Boolean/TYPE "luaclj.proteus.Containers$B"
                   Long/TYPE "luaclj.proteus.Containers$L"
                   Double/TYPE "luaclj.proteus.Containers$D"
                   "luaclj.proteus.Containers$O")
                vs)
        ks (map
             (fn [k type]
